...
const CONTACT_FORM_ID = 'xlgoyngq'; // Formspree Contact Form ID
...
document.addEventListener('click', e => {
  ...
  formData.append('Project Brief', brief);
  formData.append('_subject', `New Enquiry from ${firstName} ${lastName} - Alpha Geo Solution`);
  ...
  .then(res => {
    if (res.ok) {
      ...
      const msg = encodeURIComponent(
        `Hi Alpha Geo Solution!\n\nNew enquiry from website:\n\nName: ${firstName} ${lastName}\nEmail: ${email}\nOrganisation: ${org}\nClient Type: ${clientType}\nService: ${service}\n\nProject Brief:\n${brief}\n\nPlease get in touch. Thank you.`
      );
      window.open(`https://wa.me/918290830989?text=${msg}`, '_blank');
  ...
  .catch(() => {
    btn.textContent = 'Error! Try WhatsApp directly';
    btn.style.background = '#dc2626';
    btn.disabled = false;
...
const NEWSLETTER_FORM_ID = 'xykbqjeg'; // Formspree Newsletter Form ID
